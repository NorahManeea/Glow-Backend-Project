...
export const getProducts = async (pageNumber = 1, limit = 8) => {
...
export const updateProduct = async (id: string, updatedProduct: ProductDocument) => {
...
export const createNewProduct = async (newProduct: ProductDocument) => {
  ...
  if (productExist) {
    const error = createHttpError(409, 'Product already exists with this product name')
    throw error
  }
